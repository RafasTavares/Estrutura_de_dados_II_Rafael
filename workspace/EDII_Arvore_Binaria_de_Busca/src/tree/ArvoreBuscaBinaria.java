...
public class ArvoreBuscaBinaria {
	...
	/// 1° QUESTÃO INSERIR A 'HORA'
	public void insere(No no, int valor) {
		// Se o valor for igual insiro a hora no meio 
		if (no.getValor() == valor) {

			// Não esta inserindo a hora, por isso estou passando a hora corrente sem conversão
...
/// Inserir
	...
	public void remove(No no, No pai, int valor) {
		...
		// se o no em questao possui o valor a ser removido
			...
			// caso onde o no possui 1 filho
				...
				// se nao hÃ¡ subarvore Ã  direita, pegue o antecessor
				if (no.getEsquerda() != null) {
					if (no == this.raiz) {
				...
				// se nao hÃ¡ subarvore Ã  esquerda, pegue o sucessor
				else {
					if (no == this.raiz)
	...
	public void remove(int valor) {
		// caso em que a arvore estÃ¡ vazia
		if (this.getRaiz() == null)
			;
		// caso em que hÃ¡ apenas a raiz
